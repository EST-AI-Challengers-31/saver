...
function Import-RuntimeConfig {
    ...
    if ([string]::IsNullOrWhiteSpace($EncodedConfig)) { throw 'Runtime configuration was not supplied by GitHub Actions.' }
    $json = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($EncodedConfig))
...
function Assert-RequiredEnv {
    ...
    if ($missing.Count -gt 0) { throw ('Required environment variables are missing: ' + ($missing -join ', ')) }
}

function Initialize-DockerConfig {
    param([string]$ProjectRoot, [string]$User, [string]$Token)
    ...
    $credential = [System.Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes("${User}:${Token}"))
    $config = @{ auths = @{ 'ghcr.io' = @{ auth = $credential } } } | ConvertTo-Json -Depth 5
...
function Wait-WebHealth {
    ...
    while ((Get-Date) -lt $deadline) {
        try {
            ...
            if ($response.StatusCode -ge 200 -and $response.StatusCode -lt 400) { Write-Host "Web health passed: $url"; return }
...
try {
    ...
    Assert-RequiredEnv -Names @('MARIADB_DATABASE','MARIADB_USER','MARIADB_PASSWORD','MARIADB_ROOT_PASSWORD','SPRING_DATASOURCE_URL','SPRING_DATASOURCE_USERNAME','SPRING_DATASOURCE_PASSWORD','AI_BASE_URL')
    if ([string]::IsNullOrWhiteSpace($BackendImage) -or [string]::IsNullOrWhiteSpace($AiImage)) { throw 'BackendImage and AiImage are required.' }
    ...
    $DockerConfigPath = Initialize-DockerConfig -ProjectRoot $AppPath -User $RegistryUser -Token $RegistryToken
    ...
    if (($running -contains 'mariadb') -and (Test-Path -LiteralPath $BackupPath)) {
        ...
        if ($LASTEXITCODE -ne 0) { throw 'MariaDB backup failed.' }
    } else { Write-Host 'MariaDB backup skipped because the service is not running yet.' }
    & docker compose -f $ComposePath pull backend ai caddy
    if ($LASTEXITCODE -ne 0) { throw 'Docker image pull failed.' }
    & docker compose -f $ComposePath up -d --remove-orphans
    if ($LASTEXITCODE -ne 0) { throw 'Docker Compose up failed.' }
    $hostPort = if ($env:DAHUM_HOST_PORT) { [int]$env:DAHUM_HOST_PORT } else { 9000 }
    Wait-WebHealth -Port $hostPort -TimeoutSeconds 150
    Write-Host 'Python-first Dahum deployment completed.'
} finally {
    if ($DockerConfigPath -and (Test-Path -LiteralPath $DockerConfigPath)) { Remove-Item -LiteralPath $DockerConfigPath -Recurse -Force -ErrorAction SilentlyContinue }
}
