...
type Screen = "login" | "upload" | "loading" | "result" | "detail" | "parentGuide" | "failed" | "serviceIntro" | "history" | "historyDetail"
...
function MenuSheet({ onClose, onShowServiceIntro, onShowHistory, onShowTerms, onShowPrivacy }: { onClose: () => void; onShowServiceIntro: () => void; onShowHistory: () => void; onShowTerms: () => void; onShowPrivacy: () => void }) {
  const items = [{ label: "분석 이력", action: onShowHistory }, { label: "서비스 소개", action: onShowServiceIntro }, { label: "이용약관", action: onShowTerms }, { label: "개인정보처리방침", action: onShowPrivacy }]
  return <div className="absolute inset-0 z-40 flex flex-col justify-end"><button aria-label="메뉴 닫기" className="absolute inset-0 bg-black/40" onClick={onClose} /><div className="relative bg-white rounded-t-[24px] pb-[18px]"><div className="flex justify-center pt-[12px] pb-[4px]"><div className="bg-[#e2e8f0] rounded-full h-[4px] w-[36px]" /></div><div className="flex items-center justify-between px-[24px] py-[16px]"><div className="flex items-center gap-[10px]"><img src={imgSymbol} alt="" className="size-[28px] object-contain" /><p className="font-['Pretendard'] font-bold text-[#1b3a5c] text-[18px]">메뉴</p></div><button onClick={onClose} className="size-[32px] rounded-full bg-[#f1f5f9] text-[#64748b]">×</button></div><div className="mx-[24px] h-px bg-[#f1f5f9]" />{items.map((item) => <button key={item.label} onClick={item.action} className="flex items-center justify-between w-full px-[24px] py-[17px]"><p className="font-['Pretendard'] font-semibold text-[#1b3a5c] text-[15px]">{item.label}</p><span className="text-[#94a3b8]">›</span></button>)}<p className="px-[24px] pt-[6px] font-['Pretendard'] text-[#94a3b8] text-[12px]">닿음 v1.0.0 · Python-first analysis</p></div></div>
}
...
export default function App() {
  ...
  return <div className="flex items-start justify-center bg-[#e8edf5]" style={{ minHeight: "100dvh" }}><div className="relative bg-[#f8fafb] w-full flex flex-col" style={{ maxWidth: 430, height: "100dvh" }}>
    {screen === "login" && <LoginScreen onNext={() => setScreen("upload")} onShowTerms={() => setModal("terms")} onShowPrivacy={() => setModal("privacy")} />}
    ...
    {modal === "menu" && <MenuSheet onClose={() => setModal(null)} onShowServiceIntro={openServiceIntro} onShowHistory={openHistory} onShowTerms={() => setModal("terms")} onShowPrivacy={() => setModal("privacy")} />}
