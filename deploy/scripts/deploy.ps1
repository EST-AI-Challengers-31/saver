...
function Get-ConfigValue {
    ...
    if ($Values.ContainsKey($Key) -and -not [string]::IsNullOrWhiteSpace([string]$Values[$Key])) { return [string]$Values[$Key] }
    return $DefaultValue
}
...
function Initialize-DockerSshConfig {
    ...
    [System.IO.File]::WriteAllText((Join-Path $dockerConfig 'config.json'), $config, [System.Text.UTF8Encoding]::new($false))
    $env:DOCKER_CONFIG = $dockerConfig
    $env:DOCKER_HOST = 'npipe:////./pipe/dockerDesktopLinuxEngine'
}

function Invoke-Compose {
    param([string[]]$Arguments, [string]$EnvFile, [string]$ComposeFile)
    & docker compose --env-file $EnvFile -f $ComposeFile @Arguments
    if ($LASTEXITCODE -ne 0) { throw "docker compose failed: $($Arguments -join ' ')" }
}

function Wait-DahumHealthy {
    param([int]$HostPort, [int]$TimeoutSeconds = 180)
    $deadline = (Get-Date).AddSeconds($TimeoutSeconds)
    $lastMessage = 'No health response yet.'
    while ((Get-Date) -lt $deadline) {
        try {
            $status = Invoke-RestMethod -Uri "http://127.0.0.1:$HostPort/api/system/status" -TimeoutSec 10
            $lastMessage = ($status | ConvertTo-Json -Compress -Depth 5)
            if ($status.backend -eq 'UP' -and $status.database -eq 'UP' -and $status.ai -eq 'UP') {
...
function Try-RollbackImages {
    ...
    try {
        Invoke-Compose -Arguments @('pull', 'backend', 'ai') -EnvFile $EnvFile -ComposeFile $ComposeFile
        Invoke-Compose -Arguments @('up', '-d', '--remove-orphans') -EnvFile $EnvFile -ComposeFile $ComposeFile
        Wait-DahumHealthy -HostPort $HostPort -TimeoutSeconds 120
...
if (-not (Test-Path $EnvPath)) { throw "Runtime env file not found: $EnvPath" }
if (-not (Test-Path $ComposePath)) { throw "Compose file not found: $ComposePath" }

$config = Read-DotEnv -Path $EnvPath
$hostPort = [int](Get-ConfigValue -Values $config -Key 'DAHUM_HOST_PORT' -DefaultValue '9000')
$publicDomain = Get-ConfigValue -Values $config -Key 'PUBLIC_DOMAIN' -DefaultValue 'yellow.it.kr'
$publicBaseUrl = Get-ConfigValue -Values $config -Key 'PUBLIC_BASE_URL' -DefaultValue "https://$publicDomain"
$moveAiRoot = Get-ConfigValue -Values $config -Key 'MOVEAI_ROOT' -DefaultValue 'C:/MOVEAI'
$autoConfigureOuterCaddy = (Get-ConfigValue -Values $config -Key 'OUTER_CADDY_AUTO_CONFIGURE' -DefaultValue 'false').ToLowerInvariant() -eq 'true'
$verifyPublicUrl = (Get-ConfigValue -Values $config -Key 'VERIFY_PUBLIC_URL' -DefaultValue 'false').ToLowerInvariant() -eq 'true'

if ([string]::IsNullOrWhiteSpace($BackendImage)) { $BackendImage = Get-ConfigValue -Values $config -Key 'BACKEND_IMAGE' -DefaultValue '' }
if ([string]::IsNullOrWhiteSpace($AiImage)) { $AiImage = Get-ConfigValue -Values $config -Key 'AI_IMAGE' -DefaultValue '' }
...
if (-not $SkipGitUpdate) {
    ...
    if ($dirty.Count -gt 0) { throw 'Server repository contains uncommitted changes. Deployment stopped.' }
    if ([string]::IsNullOrWhiteSpace($RollbackCommit)) { $RollbackCommit = (git rev-parse HEAD).Trim() }
...
Login-Ghcr -User $RegistryUser -Token $RegistryToken

try {
    Write-Host "Deploying backend image: $BackendImage"
    Write-Host "Deploying AI image: $AiImage"

    Invoke-Compose -Arguments @('config') -EnvFile $EnvPath -ComposeFile $ComposePath *> $null

    $running = & docker compose --env-file $EnvPath -f $ComposePath ps --status running --services
    if ($LASTEXITCODE -ne 0) { throw 'Could not inspect current Docker services.' }

    if ($running -contains 'mariadb') {
        Write-Host 'Backing up MariaDB before deployment...'
        & $BackupPath -EnvPath $EnvPath -ComposePath $ComposePath -RuntimePath $RuntimePath
    } else {
    ...
    Invoke-Compose -Arguments @('pull', 'backend', 'ai', 'mariadb', 'caddy') -EnvFile $EnvPath -ComposeFile $ComposePath

    Write-Host 'Starting containers without building on the mini PC...'
    Invoke-Compose -Arguments @('up', '-d', '--remove-orphans') -EnvFile $EnvPath -ComposeFile $ComposePath

    Wait-DahumHealthy -HostPort $hostPort -TimeoutSeconds 180
    ...
    try { & docker compose --env-file $EnvPath -f $ComposePath ps | Out-Host } catch {}
    try { & docker compose --env-file $EnvPath -f $ComposePath logs --tail 120 backend ai mariadb caddy | Out-Host } catch {}
    Try-RollbackImages -PreviousCommit $RollbackCommit -CurrentBackendImage $BackendImage -CurrentAiImage $AiImage -EnvFile $EnvPath -ComposeFile $ComposePath -HostPort $hostPort
    throw $deploymentError
}
finally {
    try { & docker logout ghcr.io *> $null } catch {}
}

if ($autoConfigureOuterCaddy) {
    & $PublicRoutePath -MoveAiRoot $moveAiRoot -Domain $publicDomain -DahumHostPort $hostPort
} else {
    Write-Host 'OUTER_CADDY_AUTO_CONFIGURE=false; existing MOVEAI Caddyfile was not modified.'
}

if ($verifyPublicUrl) {
    $publicHealth = Invoke-RestMethod -Uri "$publicBaseUrl/api/health" -TimeoutSec 20
    if ($publicHealth.status -ne 'UP') { throw 'Public health endpoint did not report UP.' }
